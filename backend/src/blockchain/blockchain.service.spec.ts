...
import { ACCESS_ESCROW_ABI, ERC20_ABI, FRACTIONAL_VAULT_ABI } from '../config/contracts';
...
describe('BlockchainService encoders', () => {
  ...
  it('encodeSettle round-trips sessionId + elapsedSeconds', () => {
    ...
    const [sid, elapsed] = new ethers.Interface(ACCESS_ESCROW_ABI).decodeFunctionData('settle', data);
  ...
  it('encodeOpenSession round-trips all four args', () => {
    ...
    const decoded = new ethers.Interface(ACCESS_ESCROW_ABI).decodeFunctionData('openSession', data);
  ...
  it('encodeUsdcTransfer targets the creator with the exact amount', () => {
    ...
    const [recipient, amount] = new ethers.Interface(ERC20_ABI).decodeFunctionData('transfer', data);
  ...
  it('encodeBuyShares carries the share amount', () => {
    ...
    const [shares] = new ethers.Interface(FRACTIONAL_VAULT_ABI).decodeFunctionData('buyShares', data);
    expect(shares).toBe(250n);
  });

  it('logs an unavailable log range and returns any logs already read', async () => {
    const warn = jest.spyOn(Logger.prototype, 'warn').mockImplementation();
    Object.assign(svc, {
      provider: {
        getBlockNumber: jest.fn().mockResolvedValue(50_000_000),
        getLogs: jest.fn().mockRejectedValue({
    ...
    const syncLogs = (
      svc as unknown as {
        syncLogs: (key: string, address: string, topics: string[]) => Promise<ethers.Log[]>;
      }
    ).syncLogs.bind(svc);

    await expect(syncLogs('test', '0x1111111111111111111111111111111111111111', ['0xtopic'])).resolves.toEqual([]);
    expect(warn).toHaveBeenCalledWith(
      'Log sync failed for 0x1111111111111111111111111111111111111111 (50000000-50000000): pruned history unavailable',
    );
    warn.mockRestore();
  });
